...
# Multiprocessing Moduel Variables
use_multiprocessing = False
max_worker_number = 4
threshold_for_multi = 3*max_worker_number
use_time_multi = 0
...
class DNAstrand:
    ...
    def __init__(self, interval_loading, pause_profile = "flat", degradation_time = 0, include_supercoiling = True, include_busty_promoter = True, mRNA_degradation = True): 
        ...
        if use_multiprocessing:
            self.pool = mt.Pool(max_worker_number)
    ...
    def step(self, t):
        ...
        prot = 0
        # -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
        if use_multiprocessing and len(self.RNAP_LIST) > 2 * threshold_for_multi:         # if the flag use_multiprocessing, then we will be using the multiprocessing
            recur_time = len(self.RNAP_LIST)//max_worker_number
            recur_mod = len(self.RNAP_LIST)-recur_time*max_worker_number
            for i in range(recur_time):
                processes = [self.pool.apply_async(self.RNAP_LIST[i*max_worker_number+j].step, args=(t, stepping[i*max_worker_number+j])) for j in range(max_worker_number)]
                for k in range(max_worker_number):
                    prot += processes[k].get()
            for i in range(recur_mod):
                i_t = recur_time*max_worker_number + i
                prot += self.RNAP_LIST[i_t].step(t, stepping[i_t])
