fishTime = (0:10:(size(fishSignal1,1)-1)*10)'; %fishTime (fishSig at pos 1 <=> time = 0
figure();
plot(fishTime,log(mean(fishSignal1,2)), 'ro');
hold on;
%%
tcrit_max = max(criticalTime, [], 'All');
criticalTime(criticalTime < 1) = NaN; %omitting all none time related values
tcrit = mean(criticalTime,'All', 'omitnan');
tcrit_min = min(criticalTime, [], 'All', 'omitnan');
%%
figure();
x = speedtcrit(:,1);
y1 = speedtcrit(:,2);
...
yline(90);
grid;
xlabel("Avg RNAP speed(nt/sec)");
ylabel("Time (sec)");
legend({"Average tcrit", "Max tcrit", "Min tcrit", "t = 90s"}, "Location", "northeast");
hold off;
%%
a1 = 12;
b1 = 15;
...
%%
%fitting first part
%fishTime = (0:30:(size(fishSignal1,1)-1)*30)'; %fishTime (fishSig at pos 1 <=> time = 0
%y = fishSignal1(3:(3+size(x,2)),:);
a2 = 8;
b = 13;
x = fishTime(a2:b);
y = mean(fishSignal1,2);
f1 = fit(x, y(a2:b), 'exp1');
plot(f1, x, y(a2:b));

%expfit = fittype('a*exp(x/b)',...
%    'independent',{'x'},...
%    'coefficients',{'a','b'});
%f2 = fit(x, y(a:b), expfit, "StartPoint", [1 1]);
%fitting second part
...
N_totalloci = 1;%100; % total number of iterations when this function is called
...
for lociID = 1:N_totalloci %each loci is independent
    ...
    for polID = min(find(exitTime(1,:)>0)):max(find(exitTime(1,:)>0)) %checks if RNAP loaded on first position(5') and how many.
        if exitTime(1,polID)>0
            ...
            if exitTime(geneLength, polID) > 0
                ...
                lifeTime = exp2rnd(mRNALL,mRNALL/2,tcrit,1,1);
            else
    ...
    for tt = 1:length(fishTime)
        for polID = 1: size(exitTime,2) %weird bound
