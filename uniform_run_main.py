...
def uniform_run_not_repressed(a):
    env = Environment(1/a, include_busty_promoter=False)
...
if __name__ == "__main__":
    ...
    #-----Uniform Run With Repression--------

    t_i = time.perf_counter()
    for i in range(sample_size):
        result = pool.map(uniform_run_repressed, alphas)
        for j in range(size):
            if result[j][0] == False:
                finished[0,j] +=1
    ...
    #-----Uniform Run Without Repression--------

    for i in range(sample_size):

        result = pool.map(uniform_run_not_repressed, alphas)
    ...
    ax.set_ylabel('Percentage of RNAP finished', fontsize=13)
    ax.plot(alphas[3:],finished_rate_repressed[3:],color='blue', marker='^', linestyle='None',linewidth=2, markersize=8,label='simulation')
    ...
    ax.plot(alphas[3:],finished_time_average_repressed[3:],color='blue', marker='^', linestyle='None',linewidth=2, markersize=6,label='simulation')
    ax.set_xticks(ticks = np.arange(0, 1.2, step = 0.05))
    ax.set_xlim([0, 0.129])
    fig.patch.set_facecolor('xkcd:white')
